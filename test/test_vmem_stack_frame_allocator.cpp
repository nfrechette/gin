...
TEST_CASE("allocate and free from vmem stack frame allocator", "[VMemStackFrameAllocator]")
{
    using namespace gin;

    const size_t BUFFER_SIZE = 16 * 1024;

    VMemStackFrameAllocator alloc(BUFFER_SIZE);
    ...
    SECTION("test commit/decommit")
    {
        {
            AllocatorFrame frame(alloc);

            void* ptr0 = alloc.Allocate(1024, 1);
            if (ptr0) memset(ptr0, 0xcd, 1024);

            REQUIRE(alloc.GetCommittedSize() == 4096);

            void* ptr1 = alloc.Allocate(8192, 1);
            if (ptr1) memset(ptr1, 0xcd, 8192);

            REQUIRE(alloc.GetCommittedSize() == 4096 * 3);
        }

        alloc.DecommitSlack(8192);

        REQUIRE(alloc.GetCommittedSize() == 8192);

        alloc.DecommitSlack(0);
...
TEST_CASE("test invalid arguments in vmem stack frame allocator", "[VMemStackFrameAllocator]")
{
    using namespace gin;

    SECTION("test initialization")
    {
        VMemStackFrameAllocator alloc;
